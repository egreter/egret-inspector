	...
	(function(t) {
		var e = (function() {
			...
			t.prototype._doPost = function(t) {
				// 基础port检查
				if (!this.port) {
					// console.warn('[Egret调试器] Port已断开，忽略消息发送');
					return;
				}
				try {
					this.port.postMessage(t);
				} catch (err) {
					// console.warn('[Egret调试器] Port发送消息失败:', err);
					this.port = null;
				}
			};
		...
		var n = (function(t) {
			...
			e.prototype._connect = function(t) {
				...
				n.onDisconnect.addListener(function() {
					// console.warn('[Egret调试器] 连接已断开，尝试重新连接...');
					e.port = null;
					// 延迟重连，避免立即重连
					setTimeout(function() {
						try {
							...
							// console.error('[Egret调试器] 重连失败:', err);
						}
					}, 1000);
				});
				this.port = n;
			...
			e.prototype._doPost = function(t) {
				// 检查port是否仍然连接
				if (!this.port) {
					// console.warn('[Egret调试器] ContentPort已断开，忽略消息发送');
					return;
				}
				try {
					this.port.postMessage(t);
				} catch (err) {
					// console.warn('[Egret调试器] ContentPort发送消息失败:', err);
					this.port = null;
				}
			};
			e.prototype._parseMsgData = function(t) {
		...
		var e = (function() {
			...
			e.start = function() {
				...
				var o = new n(location.host, function() {
					...
					// 监听重新初始化消息
					o.on('reinitialize', function() {

						e.instance.startInspectIfDevToolOpen();
					});
				});
			};
			...
			e.prototype.injectScript = function(t) {
				// 使用Chrome扩展标准方式：直接请求background使用chrome.scripting API
				...
				chrome.runtime.sendMessage({
					...
					if (chrome.runtime.lastError) {
						// console.warn('[Egret调试器] 后台脚本执行失败:', chrome.runtime.lastError);
					} else {
			...
			e.addScript = function(t) {
				...
				t.forEach(function(scriptPath) {
					var n = e.urlFormatFunc();
					var o = document.createElement('script');
					var scriptUrl = n(scriptPath);

					o.src = scriptUrl;
					o.async = false;
					o.onerror = function(err) {
						// console.error('[Egret调试器] 脚本加载失败:', scriptUrl, err);
					};
					o.onload = function() {
              ...
              (function() {
                ...
                var intervalId = setInterval(function() {
                  ...
                  if (egret && egret.devtool && egret.devtool.start) {
                    ...
                    try {
                      egret.devtool.start();
                      // console.log('[Egret调试器] 启动成功');
                      
                      // 启动后立即发送refresh消息，确保面板更新
                      setTimeout(function() {
                        if (egret.devtool.Inspector && egret.devtool.Inspector.instance) {

                          egret.devtool.Inspector.instance.port.post({name: 'refresh'});
                        }
                      }, 500);
                    ...
                    clearInterval(intervalId);
                    // console.error('[Egret调试器] 超时: 无法找到egret.devtool.start');
                  }
                }, 100);
              })();
            `);
...
(function() {
	// 检测是否是页面刷新
	var isPageRefresh = performance.navigation.type === 1;
	if (isPageRefresh) {
		// console.log('[Egret调试器] 检测到页面刷新，重新启动...');
		// 页面刷新后稍微延迟启动，确保页面完全加载
		setTimeout(function() {
